...
public class TestProbenSQL {
	...
	/**
	 * The majority of tests can be applied after the database connection can be
	 * established. This instantiation is checked afterwards in this subclass.
	 *
	...
	public class WhenNew {
		...
		void TestRangeIteratorEmpty() {
			Iterator<Probe<Integer>> it = data.range(Integer.valueOf(-5), Integer.valueOf(9)).iterator();
			assertFalse(it.hasNext());
			assertThrows(NoSuchElementException.class, () -> {
				it.next();
			});
		 ...
		 *
		 * @EnumSource(Interpretation.class) void TestResultIteratorEmpty(Interpretation
		 * i) { Iterator<Probe<Integer>> it = data.result(i).iterator();
		 * assertFalse(it.hasNext()); assertThrows(NoSuchElementException.class, () -> {
		 * it.next(); }); }
		 */
		@Test
		...
		/*
		 * We omit this test as every Object already represents a row in the database
		 * table.
		 *
		 * @Test void addByObject() { ZonedDateTime time = ZonedDateTime.now(); SQLProbe
		 * original = new SQLProbe("Sample 3",time,Integer.valueOf(79)); SQLProbe
		 * inserted = data.add(original); assertFalse(original == inserted); SQLProbe
		 * sample = getOnlyEntry(); assertTrue(sample == inserted);
		 * assertEquals("Sample 3",sample.getID()); assertEquals(time,sample.getTime());
		 * assertEquals(Integer.valueOf(79),sample.getValue());
		 *
		 * assertThrows(IllegalArgumentException.class, () -> { data.add(original); });
		 * }
		 */
		...
		class WhenAdd {
			...
			void fillDatabase() {
				ZonedDateTime time = ZonedDateTime.now();
				Stream<TestSQLProbe> sampleStream = Stream.of(new TestSQLProbe((String) null, time.plusHours(1)),
				...
				Collections.sort(samples, (Comparator<? super TestSQLProbe>) (TestSQLProbe a, TestSQLProbe b) -> a.time
						.compareTo(b.time));
			}
			...
			void testDeleteById(RepetitionInfo repetitionInfo) throws SQLException {
				...
				for (int i = 0; i != samples.size(); ++i) {
					if (i == repetitionInfo.getCurrentRepetition() - 1)
						continue;
