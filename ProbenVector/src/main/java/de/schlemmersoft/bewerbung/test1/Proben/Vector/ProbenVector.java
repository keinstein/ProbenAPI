...
/**
 * An implementation of the ProbenAPI which uses {@link Vector} for the internal
 * storage.
 *
 * @param <T>           Data type of the measurement values.
 * @param <VectorProbe> the corresponding sample class.
 *
 */
public abstract class ProbenVector<T extends Comparable<T>, VectorProbe extends GenericProbe<T>>
	...
	int findInsertionPoint(ZonedDateTime time) {
		...
		int index = Collections.binarySearch(data, createElement("", time, null), c);
		if (index < 0)
			index = -index - 1;
	...
	abstract protected VectorProbe createElement(String id, ZonedDateTime time, T value);

	/**
	 * Count the samples in the database.
	 * @return the size of the data base in samples.
	 */
	public int size() {
		return data.size();
	}
	...
	/**
	 * Copy a sample object into the database.
	 *
	 * @param sample The object to be copied.
	 * @return the copy from the database.
	 * @throws IllegalArgumentException When inserting the sample would lead to a
	 *                                  duplicate identifier an instance of @link
	...
	public VectorProbe add(Probe<T> sample) throws IllegalArgumentException {
		...
		if (pos < data.size()) {
			data.add(pos, createElement(sample.getID(), sample.getTime(), sample.getValue()));

			return data.elementAt(pos);
		} else {
			data.add(createElement(sample.getID(), sample.getTime(), sample.getValue()));
	...
	public VectorProbe add(String id, ZonedDateTime time) throws IllegalArgumentException {
		...
		if (pos < data.size()) {
			data.add(pos, createElement(id, time, null));
			return data.elementAt(pos);
		} else {
	...
	/**
	 * Create a sample in the database given by all fields.
	 *
	 * @param id    Unique identifier of the sample.
	 * @param time  Time the sample has been measured.
	 * @param value Measurement value (may be #null).
	 * @return The representation of the sample in the database.
	 */
	...
	/**
	 * Create a sample in the database given by all fields. Here, the unique
	...
	@Override
	public void remove(String id) throws IllegalArgumentException, IllegalStateException {
	...
	@Override
	public void remove(Probe<T> sample) throws IllegalArgumentException, IllegalStateException {
